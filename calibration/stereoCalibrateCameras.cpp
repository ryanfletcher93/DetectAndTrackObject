...
#define CAM1_PARAM_FILE "cameraSetup2/cam1/calibResults"
#define CAM2_PARAM_FILE "cameraSetup2/cam2/calibResults"
#define DEFAULT_SAVE_FILE "cameraSetup2/stereo/StereoCameraProperties"

#define CAM1_IPADDR "192.168.1.6"
#define CAM2_IPADDR "192.168.1.2"

#define FILE1_TEMPLATE "cameraSetup2/cam1/background1WithObjects.png"
#define FILE2_TEMPLATE "cameraSetup2/cam2/background1WithObjects.png"
#define STEREO1_FILE_PATH "cameraSetup2/cam1/image*.png"
#define STEREO2_FILE_PATH "cameraSetup2/cam2/image*.png"
#define REPLACE_STR "\\*"
...
vector<array<Mat, 2>> getImagesFromFile(int numImgs, string imgPath1, string imgPath2, 
    ...
    for (int i=0; i<numImgs; i++) {
        filePath1 = std::regex_replace(imgPath1, r, std::to_string(i));
        filePath2 = std::regex_replace(imgPath2, r, std::to_string(i));


        tempMat1 = cv::imread(filePath1, CV_LOAD_IMAGE_COLOR);
        tempMat2 = cv::imread(filePath2, CV_LOAD_IMAGE_COLOR);

        // Check if file exists
        if (true) {//access( filePath1.c_str(), F_OK ) != -1 && access( filePath2.c_str(), F_OK ) != -1) {
